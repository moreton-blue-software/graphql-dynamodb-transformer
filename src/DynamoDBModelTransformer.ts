...
import {
  ...
  getUpdatedAtFieldName,
} from "./ModelDirectiveArgs";
...
export class DynamoDBModelTransformer extends Transformer {
  ...
  public after = (ctx: TransformerContext): void => {
    ...
    if (ddbMetata) {
      Object.entries(ddbMetata.hoistedRequestMappingContent || {}).forEach(
        ([resourceId, hoistedContentGenerator]: [
          string,
          () => string | void
        ]) => {
  ...
  public object = (
    ...
    if (
      isTypeNameReserved &&
      ctx.featureFlags.getBoolean("validateTypeNameReservedWords", true)
    ) {
    ...
    const nonModelArray: ObjectTypeDefinitionNode[] = getNonModelObjectArray(
      def,
      ctx,
    ...
    ctx.setResource(
      iamRoleLogicalID,
      this.resources.makeIAMRole(typeName, this.opts.SyncConfig)
    );
    ctx.mapResourceToStack(stackName, iamRoleLogicalID);
    ctx.setResource(
      dataSourceRoleLogicalID,
      this.resources.makeDynamoDBDataSource(
        tableLogicalID,
        iamRoleLogicalID,
        typeName,
        isSyncEnabled
    ...
    const streamArnOutputId = `GetAtt${ModelResourceIDs.ModelTableStreamArn(
      typeName
    )}`;
    ctx.setOutput(
    ...
    if (isSyncEnabled) {
      ...
      const newFields = [
        ...
        makeField(
          "_lastChangedAt",
          [],
          wrapNonNull(makeNamedType("AWSTimestamp"))
        ),
      ];
  ...
  private addTimestampFields(
    ...
    const existingCreatedAtField = def.fields.find(
      (f) => f.name.value === createdAtField
    );
    const existingUpdatedAtField = def.fields.find(
      (f) => f.name.value === updatedAtField
    );
    ...
    if (
      !DynamoDBModelTransformer.isTimestampCompatibleField(
        existingCreatedAtField
      )
    ) {
      console.log(
        `${def.name.value}.${
          existingCreatedAtField.name.value
        } is of type ${getBaseType(
    ...
    if (
      !DynamoDBModelTransformer.isTimestampCompatibleField(
        existingUpdatedAtField
      )
    ) {
      console.log(
        `${def.name.value}.${
          existingUpdatedAtField.name.value
        } is of type ${getBaseType(
    ...
    const newObj: ObjectTypeDefinitionNode = {
      ...
      fields: [
        ...obj.fields,
        ...(createdAtField && !existingCreatedAtField
          ? [
              makeField(
                createdAtField,
                [],
                wrapNonNull(makeNamedType("AWSDateTime"))
              ),
            ]
          : []), // createdAt field
        ...(updatedAtField && !existingUpdatedAtField
          ? [
              makeField(
                updatedAtField,
                [],
  ...
  private createMutations = (
    ...
    const directiveArguments: ModelDirectiveArgs =
    ...
    const timestampFields = {
      createdAtField: DynamoDBModelTransformer.isTimestampCompatibleField(
        existingCreatedAtField
      )
        ? createdAtField
        : undefined,
      updatedAtField: DynamoDBModelTransformer.isTimestampCompatibleField(
        existingUpdatedAtField
      )
        ? updatedAtField
        : undefined,
    ...
    const conditionInputName =
      ModelResourceIDs.ModelConditionInputTypeName(typeName);

    // Create the mutations.
    if (shouldMakeCreate) {
      const createInput = makeCreateInputObject(
        def,
        directive,
        nonModelArray,
        ctx,
      ...
      this.addInitalizationMetadata(ctx, resourceId, () => {
        const inputObj = ctx.getType(
          createInput.name.value
        ) as InputObjectTypeDefinitionNode;
        if (inputObj) {
          return this.resources.initalizeDefaultInputForCreateMutation(
            inputObj,
            timestampFields
          );
        }
      ...
      const args = [
        makeInputValueDefinition(
          "input",
          makeNonNullType(makeNamedType(createInput.name.value))
        ),
      ];
      if (this.supportsConditions(ctx)) {
        args.push(
          makeInputValueDefinition(
            "condition",
            makeNamedType(conditionInputName)
          )
        );
      }
      mutationFields.push(
        makeField(
          createResolver.Properties.FieldName.toString(),
          args,
    ...
    if (shouldMakeUpdate) {
      const updateInput = makeUpdateInputObject(
        def,
        nonModelArray,
        ctx,
        isSyncEnabled
      ...
      const args = [
        makeInputValueDefinition(
          "input",
          makeNonNullType(makeNamedType(updateInput.name.value))
        ),
      ];
      if (this.supportsConditions(ctx)) {
        args.push(
      ...
      mutationFields.push(
        makeField(
          updateResolver.Properties.FieldName.toString(),
          args,
          makeNamedType(def.name.value)
        )
      );
    ...
    if (shouldMakeDelete) {
      ...
      const resourceId =
        ResolverResourceIDs.DynamoDBDeleteResolverResourceID(typeName);
      ctx.setResource(resourceId, deleteResolver);
      ctx.mapResourceToStack(typeName, resourceId);
      const args = [
        makeInputValueDefinition(
          "input",
          makeNonNullType(makeNamedType(deleteInput.name.value))
        ),
      ];
      if (this.supportsConditions(ctx)) {
        args.push(
      ...
      mutationFields.push(
        makeField(
          deleteResolver.Properties.FieldName.toString(),
          args,
          makeNamedType(def.name.value)
        )
      );
  ...
  private createQueries = (
    ...
    if (isSyncEnabled) {
      ...
      const syncResourceID =
        ResolverResourceIDs.SyncResolverResourceID(typeName);
        ...
        makeField(
          ...
          [
            makeInputValueDefinition(
              "filter",
              makeNamedType(
                ModelResourceIDs.ModelFilterInputTypeName(def.name.value)
              )
            ),
          ...
          makeNamedType(
            ModelResourceIDs.ModelConnectionTypeName(def.name.value)
          )
        )
      );
    }
    ...
    if (shouldMakeGet) {
      ...
      const resourceId =
        ResolverResourceIDs.DynamoDBGetResolverResourceID(typeName);
    ...
    if (shouldMakeList) {
      ...
      const resourceId =
        ResolverResourceIDs.DynamoDBListResolverResourceID(typeName);
      ctx.setResource(resourceId, listResolver);
      ctx.mapResourceToStack(typeName, resourceId);

      queryFields.push(
        makeConnectionField(
          listResolver.Properties.FieldName.toString(),
          def.name.value
        )
      );
      this.generateFilterInputs(ctx, def);
  ...
  private createSubscriptions = (
    ...
    const createResolver = ctx.getResource(
      ResolverResourceIDs.DynamoDBCreateResolverResourceID(typeName)
    );
    const updateResolver = ctx.getResource(
      ResolverResourceIDs.DynamoDBUpdateResolverResourceID(typeName)
    );
    const deleteResolver = ctx.getResource(
      ResolverResourceIDs.DynamoDBDeleteResolverResourceID(typeName)
    );
    ...
    if (
      ...
      (subscriptionsArgument.onCreate ||
        subscriptionsArgument.onUpdate ||
        subscriptionsArgument.onDelete)
    ) {
      ...
      for (const field of Object.keys(subscriptionToMutationsMap)) {
        if (onCreate.includes(field) && createResolver) {
          subscriptionToMutationsMap[field].push(
  ...
  private generateModelXConnectionType(
    ...
    const tableXConnectionName = ModelResourceIDs.ModelConnectionTypeName(
      def.name.value
    );
  ...
  private generateFilterInputs(
    ...
    const enumFilters = makeEnumFilterInputObjects(
      def,
      ctx,
      this.supportsConditions(ctx)
    );
    ...
    const tableXQueryFilterInput = makeModelXFilterInputObject(
      def,
      ctx,
      this.supportsConditions(ctx)
    );
  ...
  private generateConditionInputs(
    ...
    if (this.supportsConditions(ctx)) {
      ...
      const tableXMutationConditionInput = makeModelXConditionInputObject(
        def,
        ctx,
        this.supportsConditions(ctx)
      );
  ...
  private setSyncConfig(ctx: TransformerContext, typeName: string) {
    ...
    if (
      ...
      resolverConfig.models[typeName]
    ) {
      const typeResolverConfig = resolverConfig.models[typeName];
      if (
        typeResolverConfig.ConflictDetection &&
        typeResolverConfig.ConflictHandler
      ) {
        syncConfig = typeResolverConfig;
      } else {
        console.warn(
          `Invalid resolverConfig for type ${typeName}. Using the project resolverConfig instead.`
        );
      }
    }
  ...
  private updateMutationConditionInput(
    ...
    if (this.supportsConditions(ctx)) {
      ...
      const tableXMutationConditionInputName =
        ModelResourceIDs.ModelConditionInputTypeName(type.name.value);

      if (this.typeExist(tableXMutationConditionInputName, ctx)) {
        const tableXMutationConditionInput = <InputObjectTypeDefinitionNode>(
          ctx.getType(tableXMutationConditionInputName)
        );

        const keyDirectives = type.directives.filter(
          (d) => d.name.value === "key"
        );
        ...
        const idField = tableXMutationConditionInput.fields.find(
          (f) => f.name.value === "id"
        );

        if (idField) {
          const reducedFields = tableXMutationConditionInput.fields.filter(
            (f) => Boolean(f.name.value !== "id")
          );
  ...
  private static isTimestampCompatibleField(
    ...
    if (
      field &&
      !(
        getBaseType(field.type) === "AWSDateTime" ||
        getBaseType(field.type) === "String"
      )
    ) {
      return false;
    }
  ...
  private addInitalizationMetadata(
    ctx: TransformerContext,
    resourceId: string,
    initCodeGenerator: () => string | void
  ): void {
    const ddbMetadata = ctx.metadata.has(METADATA_KEY)
      ? ctx.metadata.get(METADATA_KEY)
      : {};
