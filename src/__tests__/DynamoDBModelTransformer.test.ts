...
test("Test DynamoDBModelTransformer with query overrides", () => {
  ...
  expectFieldsOnInputType(createPostInput, [
    "id",
    "title",
    "createdAt",
    "updatedAt",
  ...
  expectFields(subscriptionType, [
    "onCreatePost",
    "onUpdatePost",
    "onDeletePost",
  ]);
  const subField = subscriptionType.fields.find(
    (f) => f.name.value === "onCreatePost"
  );
...
test("Test DynamoDBModelTransformer with advanced subscriptions", () => {
  ...
  const subField = subscriptionType.fields.find(
    (f) => f.name.value === "onFeedUpdated"
  );
  expect(subField.directives.length).toEqual(1);
  expect(subField.directives[0].name.value).toEqual("aws_subscribe");
  const mutationsList = subField.directives[0].arguments.find(
    (a) => a.name.value === "mutations"
...
test("Test DynamoDBModelTransformer with non-model types and enums", () => {
  ...
  const postMetaDataInputField = getFieldOnInputType(
    createPostInputType,
    "metadata"
  );
...
test("DynamoDB transformer should add default primary key when not defined", () => {
  ...
  const createPostInput: InputObjectTypeDefinitionNode =
    schema.definitions.find(
      (d) =>
        d.kind === "InputObjectTypeDefinition" &&
        d.name.value === "CreatePostInput"
    ) as InputObjectTypeDefinitionNode | undefined;
  expect(createPostInput).toBeDefined();
  const defaultIdField: InputValueDefinitionNode = createPostInput.fields.find(
    (f) => f.name.value === "id"
  );
    ...
    featureFlags: {
      getBoolean: jest
        ...
        .mockImplementation((name) =>
          name === "validateTypeNameReservedWords" ? false : undefined
        ),
...
function expectFields(type: ObjectTypeDefinitionNode, fields: string[]) {
  for (const fieldName of fields) {
    const foundField = type.fields.find(
...
function expectFieldsOnInputType(
  type: InputObjectTypeDefinitionNode,
  fields: string[]
) {
  for (const fieldName of fields) {
    const foundField = type.fields.find(
      (f: InputValueDefinitionNode) => f.name.value === fieldName
    );
    expect(foundField).toBeDefined();
  }
}

function getFieldOnInputType(
  type: InputObjectTypeDefinitionNode,
  field: string
): InputValueDefinitionNode {
  return type.fields.find(
    (f: InputValueDefinitionNode) => f.name.value === field
  );
}

function getFieldOnObjectType(
...
function getObjectType(
  ...
  return doc.definitions.find(
    (def: DefinitionNode) =>
      def.kind === Kind.OBJECT_TYPE_DEFINITION && def.name.value === type
...
function getInputType(
  ...
  return doc.definitions.find(
    (def: DefinitionNode) =>
      def.kind === Kind.INPUT_OBJECT_TYPE_DEFINITION && def.name.value === type
  ) as InputObjectTypeDefinitionNode | undefined;
}

function verifyInputCount(
  doc: DocumentNode,
  type: string,
  count: number
): boolean {
